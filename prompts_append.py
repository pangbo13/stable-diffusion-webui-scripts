...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        # checkbox_txt = gr.Checkbox(label="Show Textbox", value=True)
        emphasize = gr.Checkbox(label="Emphasize Appended Prompts", value=True)
        prompt_txt = gr.TextArea(label="Prompts")
        file = gr.File(label="File with inputs", type='bytes', visible = False)
        ...
        return [file, prompt_txt, emphasize]
    ...
    def run(self, p, data: bytes, prompt_txt: str, emphasize: bool):
        lines = []
        if prompt_txt:
        ...
        for n, line in enumerate(lines):
            ...
            state.textinfo = f"{state.job_no + 1}/{state.job_count},appended prompt(s): {line}"
            
            copy_p = copy.copy(p)
            # copy_p.do_not_save_samples = True
            if line != "-":
                if not copy_p.prompt.endswith(','):
                    copy_p.prompt += ","
                if emphasize:
                    copy_p.prompt += "((" + line + "))"
                else:
        ...
        # images = list(chain(map(lambda p:p.images,proc_images)))
        # infotexts = list(chain(map(lambda p:p.infotexts,proc_images)))
